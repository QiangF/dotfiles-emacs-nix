...
~describe-function~ and ~describe-variable~ are mapped to their respectful =helpful= functions via the variables ~counsel-describe-variable-function~ and ~counsel-describe-function-function~ in the [[*Ivy][section above]].
...
(use-package magit
  ...
  (purplg/leader-key :states 'normal
    "g" '(:which-key "git")
    "g g" '(magic-status :which-key "status")))
